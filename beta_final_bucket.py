...
"""Contains beta calculation for individual stocks"""
...
def fetch_data(symbol, start_date, input_date, mycursor, con_mydb):
	...
	stock_query = ("SELECT * FROM stock WHERE date_stamp BETWEEN %s AND %s AND symbol=%s")
...
if __name__ == "__main__":
	...
	for row in rows:
		...
		if prev_date_year is None:
			...
			static_date = datetime.datetime.strptime(static_date, "%Y%m%d").date()
		...
		if snp_percentage > float(input_x) and snp_percentage >= 0:
			...
			for ticker in ticker_list:
				...
				query = ("SELECT * FROM stock WHERE date_stamp = %s AND symbol = %s")
				...
				if rec is not None:
					...
					stock_record, snp_record = fetch_data(ticker, start_date, end_date, mycursor, con_mydb)
			...
			sorted_tickers_beta = sorted(tickers_beta, key=lambda x: tickers_beta[x][1], reverse=True)
			...
			d = [current_bucket_list.remove(key) for key in ten_per_lowest_beta if key in current_bucket_list]
			print(current_bucket_list)
			a = [current_bucket_list.append(key) for key in ten_per_highest_beta]
			print(current_bucket_list)
			current_bucket_list_string = ','.join(current_bucket_list)

			""" sumation of the closing price of current bucket list """
			current_closing_list = [tickers_beta[key][0] for key in current_bucket_list if key in tickers_beta]
			...
			if prev_rec[4] is None:
				...
				if last_inserted_id is 1:
					...
					query = ("UPDATE temp_portfolio SET portfolio_value=%s WHERE seq_no=%s")
					...
					prev_portfolio_value = sum(prev_closing_list)
					return_percentage_change = ((prev_portfolio_value - prev_prev_portfolio_value)/prev_prev_portfolio_value)*100
					query = ("UPDATE temp_portfolio SET portfolio_value=%s, return_percentage=%s WHERE seq_no=%s")
					values = (prev_portfolio_value, return_percentage_change, prev_rec[0])
					mycursor.execute( query, values )
					con_mydb.commit()
			else:
			...
			return_percentage_change = ((portfolio_value - prev_portfolio_value)/prev_portfolio_value)*100
